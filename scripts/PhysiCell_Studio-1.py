...
class PhysiCellXMLCreator(QWidget):
    def __init__(self, config_file, studio_flag, skip_validate_flag, rules_flag, model3D_flag, exec_file, parent = None):
        ...
        # print("PhysiCell_Studio.py: self.pmb_config_dir = ",self.pmb_config_dir)
        # sys.exit(1)
        ...
        # print("PhysiCell_Studio.py: first_substrate_name=",substrate_name)
        ...
        # print("PhysiCell_Studio.py: first_cell_def_name=",cd_name)
        logging.debug(f'PhysiCell_Studio.py: first_cell_def_name= {cd_name}')
        ...
        print("PhysiCell_Studio.py: ",self.xml_root.find(".//cell_definitions//cell_rules"))
        ...
        self.current_dir = os.getcwd()
        # print("PhysiCell_Studio.py: self.current_dir = ",self.current_dir)
        logging.debug(f'PhysiCell_Studio.py: self.current_dir = {self.current_dir}')
        ...
        if self.studio_flag:
            logging.debug(f'PhysiCell_Studio.py: creating ICs, Run, and Plot tabs')
            ...
            logging.debug(f'PhysiCell_Studio.py: calling vis_tab.substrates_cbox_changed_cb(2)')
...
PhysiCell Studio is provided "AS IS" without warranty of any kind. &nbsp; In no event shall the Authors be liable for any damages whatsoever.<br>
    ...
    def open_as_cb(self):
        ...
        logging.debug(f'\nPhysiCell_Studio.py: open_as_cb():  full_path_model_name ={full_path_model_name}')
    ...
    def save_as_cb(self):
        ...
        try:
            ...
            # print("\n\n ===================================")
            print("PhysiCell_Studio.py:  save_as_cb: writing to: ",self.current_xml_file)
    ...
    def save_cb(self):
        try:
            ...
            # filePath = QFileDialog.getOpenFileName(self,'',".",'*.xml')
            # print("PhysiCell_Studio.py:  save_cb: writing to: ",self.config_file)
            ...
            # print("\n\n ===================================")
            # print("PhysiCell_Studio.py:  save_cb: writing to: ",out_file)
            print("PhysiCell_Studio.py:  save_cb: writing to: ",self.current_xml_file)
    ...
    def select_plot_output_cb(self):
        ...
        # logging.debug(f'\nPhysiCell_Studio.py: select_plot_output_cb():  full_path_model_name ={full_path_model_name}')
    ...
    def view3D_cb(self, action):
        logging.debug(f'PhysiCell_Studio.py: view3D_cb: {action.text()}, {action.isChecked()}')
    ...
    def load_model(self,name):
        ...
        logging.debug(f'PhysiCell_Studio.py: load_model(): self.current_xml_file= {self.current_xml_file}')

        logging.debug(f'PhysiCell_Studio.py: load_model(): {self.xml_root.find(".//cell_definitions//cell_rules")}')
        print(f'PhysiCell_Studio.py: load_model(): {self.xml_root.find(".//cell_definitions//cell_rules")}')
    ...
    def message(self, s):
        ...
        print('PhysiCell_Studio.py: message(): ',s)
...
def main():
    ...
    try:
        ...
        if args.studio:
            logging.debug(f'PhysiCell_Studio.py: Studio mode: Run,Plot,Legend tabs')
            studio_flag = True
            # print("done with args.studio")
        if args.three:
            logging.debug(f'PhysiCell_Studio.py: Assume a 3D model')
            model3D_flag = True
            # print("done with args.three")
        # if args.rules:
        #     logging.debug(f'PhysiCell_Studio.py: Show Rules tab')
        #     rules_flag = True
        if args.skip_validate:
            logging.debug(f'PhysiCell_Studio.py: Do not validate the config file (.xml)')
            skip_validate_flag = True
        # print("args.config= ",args.config)
        if args.config:
            logging.debug(f'PhysiCell_Studio.py: config file is {args.config}')
            # sys.exit()
            config_file = args.config
            if (len(config_file) > 0) and Path(config_file).is_file():
                logging.debug(f'PhysiCell_Studio.py: open_as_cb():  filePath is valid')
